	...
	passport.use(new LocalStrategy(
	  {
	    ...
	    passwordField: 'password'
			...
			loginFunc(userid,password,function(err,user) {
...
module.exports.login = function(req,res,next) {
	// authenticate()用作路由中间件来对请求进行身份验证。
	passport.authenticate('local', function(err, user) {
		...
		// 获取角色信息
		var token = jwt.sign({"uid":user.userid,"rid":user.isAdmin}, jwt_config.get("secretKey"), {"expiresIn": jwt_config.get("expiresIn")});
